...
def get_all_train_file(args, skim):
    ...
    if args.dataset == 'sketchy_extend':
        if args.test_class == "test_class_sketchy25":
            ...
            shot_dir = "zeroshot0"
        elif args.test_class == "test_class_sketchyam":
            shot_dir = "zeroshot2"
...
# def load_object_feature(image_path):
#     feature = [None]
#     FIELDNAMES = ['image_file', 'image_id', 'image_w', 'image_h', 'num_boxes', 'boxes', 'features']
#     root_dir = os.path.split(image_path)
#     file_path, file_name = root_dir[0], image_path.split('/')[-2] + '/' + root_dir[1]
#     tsv_file = file_path + '.tsv'
#     tsv_file = r'D:\ZJU\research\sketch\sketch_retrieval\dataset\dataset\Sketchy\EXTEND_image_sketchy_ready/camel.tsv'
#     file_name = 'camel/n02437136_5405.jpg'
#
#     tsv = pd.read_csv(tsv_file, sep='\t', header=None, names=FIELDNAMES)
#     index = tsv[tsv['image_file'] == file_name].index
#     item = tsv.loc[index]
#
#     for field in ['boxes', 'features']:
#         data = str(item[field][index[0]])
#         buf = base64.b64decode(data[1:])
#         temp = np.frombuffer(buf, dtype=np.float32)
#         reshaped_temp = temp.reshape((int(item['num_boxes'].iloc[0]), -1))
#         # 创建副本并进行修改
#         item_copy = item.copy()
#         item_copy.at[index[0], field] = reshaped_temp
#
#     feature[0] = item_copy['features'][index[0]]
#     data_out = torch.Tensor(np.array(feature))
#     data_out = torch.squeeze(data_out, 0)
#     return data_out
